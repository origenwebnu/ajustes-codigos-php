(function () {
    ...
    function createOverviewItem(slot) {
        const item = document.createElement('li');
        item.className = 'woocommerce-order-overview__' + slot + ' cr-overview-item cr-overview-item--' + slot;
        return item;
    }
    ...
    function restructureOrderOverview(root) {
        ...
        const collected = Array.from(list.querySelectorAll('li')).map(function (item) {
            ...
            return entry.slot !== '' && entry.slot !== 'email';
        });
        ...
        slots.forEach(function (slot) {
            ...
            if (!entry && slot === 'payment') {
                entry = {
                    slot: 'payment',
                    item: createOverviewItem('payment-method'),
                    value: paymentValue
                };
            }
            ...
            let value = slot === 'payment' ? (paymentValue || entry.value) : entry.value;
            if (slot === 'date') {
    ...
    function cleanOrderProductNameCell(nameCell) {
